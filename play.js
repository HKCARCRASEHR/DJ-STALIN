...
module.exports = {
  async play(song, message, client, filters) {
    ...
    if (!song) {
      ...
      const endembed = new MessageEmbed().setColor("#FF0000")
        .setAuthor(`Music Queue ended.`, "https://i.kym-cdn.com/photos/images/original/000/220/724/1324499344002.gif")
  ...
  let thumb;
    if (song.thumbnail === undefined) thumb = "https://i.kym-cdn.com/photos/images/original/000/220/724/1324499344002.gif";
    else thumb = song.thumbnail.url;

  try {
      let embed = new MessageEmbed()
      .setColor("#ff0000")
      .setAuthor(`Red Army Choir now playing ${song.title}`,'https://i.kym-cdn.com/photos/images/original/000/220/724/1324499344002.gif')
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "⏯":
          ...
          if (queue.playing) {
            ...
            queue.textChannel.send(` ▶ Resume it now! Now!`).catch(console.error);
          }
          break;
        ...
        case "🔉":
          ...
          queue.textChannel
            .send(` 🔉 the volume is now ${queue.volume}% my comrade`)
            .catch(console.error);
          break;
        ...
        case "⏹":
          ...
          if (!canModifyQueue(member)) return;
          queue.songs = [];
          queue.textChannel.send(`⏹ stop it! cyka blayt!`).catch(console.error);
          try {
            queue.connection.dispatcher.end();
          } catch (error) {
            console.error(error);
        ...
        if (!queue)
            return message.channel
                  .send("There is no queue my comrade.")
                  .catch(console.error);
          if (!canModifyQueue(member)) return;
        ...
        let nowPlaying = new MessageEmbed()
          .setAuthor('Now playing','https://i.kym-cdn.com/photos/images/original/000/220/724/1324499344002.gif','http://harmonymusic.tk')
      ...
      if (ms > 0 && ms<10000) {
        nowPlaying.addField("\u200b", "**``[" + createBar((ms == 0 ? seek : ms), seek, 25, "▬", "🔘")[0] + "]``**\n**" + "\n[" + new Date(seek * 1000).toISOString().substr(11, 8) + " / " + (ms == 0 ? " ◉ LIVE" : new Date(ms * 1000).toISOString().substr(11, 8))+ "]**" + "\n" + "\n **Time Remaining:**" + "``" + new Date(left * 1000).toISOString().substr(11, 8) + "``", false );
          ...
          .setAuthor("Searching...", "https://i.kym-cdn.com/photos/images/original/000/220/724/1324499344002.gif").setFooter("Lyrics")
          .setColor("#FF0000")
